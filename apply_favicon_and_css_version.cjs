...
function processHTMLFile(filePath) {
  ...
  // 1. Update/Add Favicon with cache-bust ?v=6.4
  ...
  if (relIconMatch) {
    fileContent = fileContent.replace(relIconRegex, '<link rel="icon" type="image/png" href="/favicon.png?v=6.4">');
  } else {
    // If not found, inject in head
    fileContent = fileContent.replace(/<\/head>/, '  <link rel="icon" type="image/png" href="/favicon.png?v=6.4">\n</head>');
  }
  ...
  if (!fileContent.includes('rel="apple-touch-icon"')) {
    ...
    fileContent = fileContent.replace(
      'href="/favicon.png?v=6.4">',
      'href="/favicon.png?v=6.4">\n  <link rel="apple-touch-icon" href="/favicon.png?v=6.4">'
    );
  } else {
    const appleIconRegex = /<link rel="apple-touch-icon"[^>]*href="[^"]*"[^>]*>/g;
    fileContent = fileContent.replace(appleIconRegex, '<link rel="apple-touch-icon" href="/favicon.png?v=6.4">');
  }

  // 3. Update style.css cache-bust version to v=6.4
  const cssRegex = /style\.css\?v=[0-9.]+/g;
  fileContent = fileContent.replace(cssRegex, 'style.css?v=6.4');

  // 4. Update logo images in header and footer to use /favicon.png?v=6.4
  ...
  fileContent = fileContent.replace(logoImgRegex, (match) => {
    ...
    return '<img src="/favicon.png?v=6.4" alt="Fancy Text Logo" class="logo-img" width="32" height="32" style="border-radius:8px;">';
  });
